...
class ShadowKVCache_CPU_DISK:
    ...
    def __init__(self, 
        ...
        **kwargs
        ...
        if self.offload_device == 'disk':
            from models.disk_cache import DiskCache
            self.v_cache_disk = DiskCache(
                cache_dir, 
                config.num_hidden_layers,
                batch_size,
                config.num_key_value_heads,
    ...
    def get_svd(self, new_k_cache, layer_idx):
        ...
        self.svd_time_list.append(elapsed_time)
    ...
    def prefill_kv_cache(self,
        ...
        if self.offload_device == 'disk':
            self.v_cache_disk.write(
                layer_idx, 
                self.prefilled_batch, 
                new_v_cache[:, :, :self.max_ctx_chunks_len].reshape(bsz, self.num_key_value_heads, max_ctx_chunks, self.chunk_size*self.head_dim), # [bsz, num_kv_heads, max_ctx_chunks, chunk_size*head_dim]
            )
        else:
            self.v_cache_cpu[layer_idx][self.prefilled_batch:self.prefilled_batch + bsz, :, :max_ctx_chunks].copy_(
                new_v_cache[:, :, :self.max_ctx_chunks_len].reshape(bsz, self.num_key_value_heads, max_ctx_chunks, self.chunk_size*self.head_dim), non_blocking=True) # [bsz, num_kv_heads, max_ctx_chunks, chunk_size*head_dim]
        ...
        # if self.logger is not None:
            # self.logger.info(f"prefill_local {self.prefill_local}, chunks {self.chunks}, chunk_size {self.chunk_size}, incoming {incoming}")
        ...
        self.sparse_end = self.prefill_local + self.outlier_chunk*self.chunk_size + self.sparse_budget
        if self.logger is not None:
            self.logger.info(f"prefill_local {self.prefill_local}, outlier_chunk {self.outlier_chunk}, sparse_start {self.sparse_start}, sparse_end {self.sparse_end}")
        ...
        elapsed_time = start.elapsed_time(end)
        self.svd_time_list2.append(elapsed_time)
    ...
    def get_value_cache(self, layer_idx, position_ids):
        # if self.logger is not None:
            # self.logger.info(f"get_value_cache: layer_idx {layer_idx}, position_ids {position_ids.shape}")
        if self.offload_device == 'disk':
            # gather value cache
            self.v_cache_disk.read(
                layer_idx, 
                position_ids, 
                self.v_cache_buffer,
                slice(self.sparse_start, self.sparse_end)
            )
        else:
            shadowkv.gather_copy_with_offsets(self.v_cache_cpu[layer_idx], self.v_cache_buffer[layer_idx], self.temp, 
                                            self.offsets, self.cnts, self.signals, self.batch_size, self.num_key_value_heads, 
                                            int(self.max_ctx_chunks_len*self.head_dim), int(self.sparse_budget*self.head_dim), self.kernel_offset, self.kernel_stride, self.select_sets)
