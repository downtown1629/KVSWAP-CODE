...
from .kv_cache import KV_Cache, ShadowKVCache, ShadowKVCache_CPU_DISK
...
class LLM:
    ...
    def init_kv_cache(self, sparse_budget: int, rank: int, chunk_size: int, config, **kwargs):
        if self.attn_mode == 'full':
            ...
            self.kv_cache = ShadowKVCache(config, max_length=self.max_length, device=self.device, dtype=self.dtype, 
                                          batch_size=self.batch_size, sparse_budget=sparse_budget, rank=rank, chunk_size=chunk_size)
        elif self.attn_mode.lower() == 'shadowkv_cpu':
            self.kv_cache = ShadowKVCache_CPU_DISK(config, max_length=self.max_length, device=self.device, dtype=self.dtype, 
                                                   batch_size=self.batch_size, sparse_budget=sparse_budget, rank=rank, chunk_size=chunk_size, **kwargs)
        else:
    ...
    def layer_compute(self, 
        ...
        elif isinstance(self.kv_cache, ShadowKVCache) or isinstance(self.kv_cache, ShadowKVCache_CPU_DISK):

            if q_len >= 2*1024: # prefill
                ...
                with torch.cuda.stream(get_value_stream):
                    ...
                    # start_event.record(get_value_stream)
                    value_states = self.kv_cache.get_value_cache(layer_idx, position_ids)
                    # end_event.record(get_value_stream)
                    
                # gather key cache from GPU and RoPE it (should be hide by CPU offloading time)
                key_states = self.kv_cache.get_key_cache(layer_idx=layer_idx, position_ids=position_ids, 
                                                         rope_func=self.apply_rotary_pos_emb_single, cos_sin_cache=self.cos_sin_cache)

                curr_stream.wait_stream(get_value_stream)
                # wait for value_states to be ready
                # end_event.synchronize()
                # elapsed_time_ms = start_event.elapsed_time(end_event)
                
                # transfer_size = value_states.numel() * value_states.element_size() / (1024**2) # MB
                # transfer_size = value_states.shape[0] * value_states.shape[1] * value_states.shape[3] * self.kv_cache.sparse_budget
                # transfer_size = transfer_size * value_states.element_size() / (1024**2) # MB
                # bw = transfer_size / (elapsed_time_ms / 1000) # MB/s
                
                # if hasattr(self.kv_cache, "logger") and self.kv_cache.logger is not None:
                    # self.kv_cache.logger.info(f"Layer {layer_idx} | Transfer {value_states.shape} elements ({transfer_size:.2f} MB) in {elapsed_time_ms:.2f} ms | Bandwidth: {bw:.2f} MB/s")
    ...
    def batch_generate(self, input_ids: torch.Tensor, gen_len: int = 256, temperature: float = 0.0, top_p: float = -1, top_k :int = 50, verbose: bool = False, benchmark: bool = False, cont: bool = False):
        ...
        # if hasattr(self.kv_cache, "logger") and self.kv_cache.logger is not None:
        #     self.kv_cache.logger.info(f"Prefill {input_ids.size(1)} tokens in {prefill_time*1000:.2f} ms")
        ...
        if benchmark == True:
            return self.decode(generated_ids, skip_special_tokens=True), self.batch_size * n / (end - start), prefill_time*1000, (end - start)*1000
